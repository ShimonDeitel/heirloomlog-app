import XCTest
@testable import Heirloomlog

final class HeirloomlogTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testFreeLimitBlocksAdd() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(Heirloom(), isPro: false)
        }
        let blocked = store.add(Heirloom(), isPro: false)
    ...
    @MainActor func testProBypassesLimit() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(Heirloom(), isPro: true)
        }
        let added = store.add(Heirloom(), isPro: true)
        XCTAssertTrue(added)
    }
